...
const OrderManagement = () => {
    ...
    const shareWhatsApp = (order) => {
        ...
        const date = new Date(order.booking_date || order.bookingDate).toLocaleDateString('en-GB');
        
        let itemsText = '';
        order.items.forEach((item, index) => {
            itemsText += `${index + 1}. *${item.name}* - ${item.qty} ${item.unit}${item.price ? ` (@ Rs ${item.price})` : ''}%0A`;
        });

        const text = `📦 *BILAL VETERINARY CLINIC*%0A*Official Supply Order*%0A%0A---------------------------------------%0A🆔 *Order ID:* #ORD-${orderId}%0A📅 *Date:* ${date}%0A🏢 *Supplier:* ${order.supplier}%0A---------------------------------------%0A%0A📋 *ITEMS LIST:*%0A${itemsText}%0A---------------------------------------%0A${order.notes ? `📝 *Remarks:* ${order.notes}%0A%0A` : ''}*Generated by Bilal Vet System*`;
        
        let phoneNumber = order.contact?.replace(/\D/g, '') || '';
        // If it starts with 0 (local Pak number), convert to 92
        if (phoneNumber.startsWith('0')) {
            phoneNumber = '92' + phoneNumber.slice(1);
        }
        // If it's already international but has + (already removed by \D), it stays same
                            ...
                            <div id="printable-order" style={{ flex: 1, overflowY: 'auto', padding: '50px', background: 'white', color: '#1e293b' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '3px solid #1e293b', paddingBottom: '20px', marginBottom: '30px', alignItems: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                                        <img src={logo} alt="Bilal Vet" style={{ height: '70px', objectFit: 'contain' }} />
                                        <div>
                                            <h1 style={{ fontSize: '1.8rem', fontWeight: 950, letterSpacing: '-1px', color: '#1e293b', margin: 0 }}>BILAL VETERINARY CLINIC</h1>
                                            <p style={{ fontSize: '0.75rem', fontWeight: 700, color: '#94a3b8', margin: '4px 0 0 0' }}>Near HBL Bank, Khoka Market, Hasilpur | 0305-6699899</p>
                                        </div>
                                    </div>
                                ...
                                <div style={{ marginTop: '80px', display: 'flex', justifyContent: 'space-between' }}>
                                    ...
                                    <div style={{ textAlign: 'center' }}>
                                        ...
                                        <p style={{ fontSize: '0.65rem', fontWeight: 700, color: '#94a3b8' }}>Bilal Veterinary Clinic</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
