...
const Trash = () => {
    ...
    const fetchTrash = async () => {
        ...
        try {
            ...
            if (activeTab === 'sales') {
                query = supabase.from('sales').select('*, sale_items(*, inventory(name))').not('deleted_at', 'is', null);
            }
    ...
    const handleRestore = async (id) => {
        ...
        try {
            const { error } = await supabase.from(activeTab).update({ deleted_at: null }).eq('id', id);
            if (error) throw error;
    ...
    return (
        <div style={{ height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '20px', padding: window.innerWidth <= 768 ? '10px' : '25px', backgroundColor: '#f8fafc' }}>
            ...
            <header style={{ 
                ...
                <div style={{ 
                    ...
                    {['sales', 'inventory', 'suppliers', 'orders'].map(tab => (
                        <button key={tab} onClick={() => setActiveTab(tab)} style={{ flex: window.innerWidth <= 768 ? 1 : 'none', padding: '10px 15px', border: 'none', borderRadius: '10px', fontWeight: 800, fontSize: '0.75rem', cursor: 'pointer', background: activeTab === tab ? 'white' : 'transparent', color: activeTab === tab ? '#2563eb' : '#64748b', boxShadow: activeTab === tab ? '0 4px 6px -1px rgba(0,0,0,0.05)' : 'none', whiteSpace: 'nowrap' }}>
                            {tab.toUpperCase()}
                        </button>
                    ))}
                </div>
            ...
            <div style={{ flex: 1, background: 'transparent', display: 'flex', flexDirection: 'column' }}>
                {isLoading ? (
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'white', borderRadius: '24px' }}><Loader2 className="animate-spin" size={40} color="#2563eb" /></div>
                                        ...
                                        <div style={{ padding: '15px', display: 'flex', flexDirection: 'column', gap: '15px' }} onClick={() => setExpandedId(isExpanded ? null : item.id)}>
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                    <div style={{ padding: '8px', background: '#f1f5f9', borderRadius: '10px' }}>
                                                        {activeTab === 'sales' ? <History size={18} color="#2563eb" /> : activeTab === 'inventory' ? <Box size={18} color="#059669" /> : activeTab === 'orders' ? <ShoppingCart size={18} color="#6366f1" /> : <Truck size={18} color="#2563eb" />}
                                                    </div>
                                                    <div>
                                                ...
                                                <span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '0.7rem', fontWeight: 800, background: daysRemaining < 7 ? '#fff1f1' : '#f0fdf4', color: daysRemaining < 7 ? '#ef4444' : '#059669' }}>{daysRemaining}D Left</span>
                                            </div>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem', color: '#64748b', fontWeight: 700 }}>
                                                ...
                                                <div style={{ display: 'flex', gap: '8px' }}>
                                                    <button onClick={(e) => { e.stopPropagation(); handleRestore(item.id); }} disabled={isActionLoading === item.id} style={{ padding: '8px 12px', background: '#ecfdf5', border: '1px solid #d1fae5', color: '#059669', borderRadius: '8px', fontSize: '0.7rem', fontWeight: 800, cursor: 'pointer' }}>RESTORE</button>
                                            ...
                                            {isExpanded && activeTab === 'sales' && (
                                                <div style={{ marginTop: '10px', padding: '15px', background: '#f8fafc', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                                    <h4 style={{ fontSize: '0.75rem', fontWeight: 950, marginBottom: '10px', color: '#2563eb', display: 'flex', alignItems: 'center', gap: '8px' }}><Receipt size={14}/> ITEMS</h4>
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', fontWeight: 900, color: '#2563eb' }}>
                                                        <span>TOTAL:</span>
                                            ...
                                            return (
                                                <React.Fragment key={item.id}>
                                                    <tr style={{ borderBottom: '1px solid #f1f5f9', cursor: 'pointer', background: isExpanded ? '#f0f9ff' : 'transparent' }} onClick={() => setExpandedId(isExpanded ? null : item.id)}>
                                                        <td style={{ padding: '15px 25px' }}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                                                <div style={{ padding: '10px', background: '#f1f5f9', borderRadius: '10px' }}>
                                                                    {activeTab === 'sales' ? <History size={20} color="#2563eb" /> : activeTab === 'inventory' ? <Box size={20} color="#059669" /> : activeTab === 'orders' ? <ShoppingCart size={20} color="#6366f1" /> : <Truck size={20} color="#2563eb" />}
                                                                </div>
                                                                <div>
                                                        ...
                                                        <td style={{ padding: '15px 25px' }}><span style={{ padding: '4px 12px', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 800, background: daysRemaining < 7 ? '#fff1f1' : '#f0fdf4', color: daysRemaining < 7 ? '#ef4444' : '#059669' }}>{daysRemaining} Days</span></td>
                                                        <td style={{ padding: '15px 25px', textAlign: 'right' }}>
                                                            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                                                                <button onClick={(e) => { e.stopPropagation(); handleRestore(item.id); }} disabled={isActionLoading === item.id} style={{ padding: '8px 12px', background: '#ecfdf5', border: '1px solid #d1fae5', color: '#059669', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 800, cursor: 'pointer' }}>RESTORE</button>
                                                        ...
                                                        <tr>
                                                            <td colSpan="4" style={{ padding: '20px 25px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                                                                <div style={{ background: 'white', borderRadius: '12px', padding: '15px', border: '1px solid #e2e8f0' }}>
                                                                    <h4 style={{ fontSize: '0.8rem', fontWeight: 950, marginBottom: '10px', color: '#2563eb', display: 'flex', alignItems: 'center', gap: '8px' }}><Receipt size={16}/> INVOICE ITEMS</h4>
                                                                    ...
                                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '15px', paddingTop: '10px', borderTop: '2px solid #f1f5f9' }}>
                                                                        ...
                                                                        <span style={{ fontWeight: 950, color: '#2563eb', fontSize: '1.1rem' }}>Rs {item.total?.toLocaleString()}</span>
                                                                    </div>
                                                                </div>
                                                            </td>
                                                        </tr>
