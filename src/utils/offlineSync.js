    ...
    for (const item of queue) {
        try {
            ...
            if (item.action === 'insert') {
                const { error: err } = await supabase.from(item.table).insert(Array.isArray(item.data) ? item.data : [item.data]);
                error = err;
            } else if (item.action === 'update') {
                const { error: err } = await supabase.from(item.table).update(item.data).eq('id', item.targetId);
                error = err;
